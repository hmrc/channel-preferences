  ...
  "Calling preference" should {
    "getVerifiedEmail" should {
      "return OK (200) with the email verification if found" in new TestSetup {
        when(
          preferenceService
            .getVerifiedEmail(any[EnrolmentKey], any[IdentifierKey], any[IdentifierValue], any[Channel])(
              any[HeaderCarrier],
    ...
    "Calling processBounce endpoint to process an email bounce event" should {
      ...
      """return OK (200) if valid Json event valid""" in new TestSetup {
        val postData: JsValue = Json.parse(s"""
                                              ...
                                              |}
    """.stripMargin)
        when(mockProcessEmail.process(any[Event])(any)).thenReturn(Future.successful(Right("")))
    ...
    """.stripMargin)
      ...
      "return OK(200) when processEmail returns success" in new TestSetup {
        when(mockProcessEmail.process(any[Event])(any))
      ...
      "return InternalServerError when processEmail returns unexpected error" in new TestSetup {
        when(mockProcessEmail.process(any[Event])(any)).thenReturn(Future.failed(UnExpectedError()))
        val preferenceController =
          new PreferenceController(
            preferenceService,
            mockEntityResolver,
    ...
    trait TestSetup {
      reset(mockProcessEmail)
      reset(mockEISContactPreference)
      ...
      val processBouncePayload: JsValue =
        Json.parse(s"""
                      |{
                      |    "subject": "bounced-email",
                      |    "eventId" : "77ed39b7-d5d8-46ed-abab-a5a8ff416dae",
